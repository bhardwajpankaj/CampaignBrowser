...
class CampaignListingView: UICollectionView {
    ...
    enum Cells: String {
        ...
        /** The cell which is used to display error and retry button. */
        case errorCell
    }
    ...
    func display(campaigns: CampaignList) {
        setData(with: ListingDataSource(campaigns: campaigns))
    }
    
    // Displays loading screen.
    func showLoadingScreen() {
        setData(with: LoadingDataSource())
    }
    
    // Displays error screen.
    func showDisplayErrorScreen() {
        setData(with: ErrorDataSource())
    }
    
    private func setData<T>(with dataSource: T) where T: UICollectionViewDataSource, T: UICollectionViewDelegate {
